...
// СЦЕНА ВЫБОРА УРОВНЯ
// ============================================================================
// Отображает сетку уровней для выбранного мира.
// Поддерживает пагинацию, отображение звёзд, блокировку уровней.
// В мире Arcade все уровни разблокированы (для тестирования).
// ============================================================================
...
export class LevelSelect extends Scene {
  ...
  async create(): Promise<void> {
    ...
    // Загружаем список ID уровней для этого мира
    ...
    // Кнопка быстрого перехода к карте миров (справа)
    ...
    // Обновление интерфейса при изменении прогресса
  ...
  private renderLevelGrid(): void {
    ...
    for (let i = 0; i < pageLevels.length; i++) {
      ...
      if (this.worldId === 'arcade') {
        // В Arcade все уровни разблокированы (для удобства тестирования)
        isLocked = false;
      } else {
        if (levelNum === 1) {
          isLocked = false;
      ...
      const levelText = this.add.text(0, -10, `${levelNum}`, {
        fontSize: '18px',
        color: '#ffffff',
        fontFamily: 'monospace',
      }).setOrigin(0.5);

      const starsText = this.add.text(0, 15, '★'.repeat(stars) + '☆'.repeat(3 - stars), {
        fontSize: '10px',
        color: '#ffcc00',
      }).setOrigin(0.5);
    ...
    const prevBtn = this.add.text(this.cameras.main.width - 170, 100, '◀', {
      fontSize: '24px',
      color: '#ffffff',
    }).setInteractive({ useHandCursor: true });
    ...
    const nextBtn = this.add.text(this.cameras.main.width - 50, 100, '▶', {
      fontSize: '24px',
      color: '#ffffff',
    }).setInteractive({ useHandCursor: true });
  ...
  private createInfoPanel(): void {
    ...
    this.infoLevelText = this.add.text(0, -50, 'Level --', {
      fontSize: '14px',
      color: '#ffffff',
      fontFamily: 'monospace',
    }).setOrigin(0.5);
    this.infoStarsText = this.add.text(0, -20, '★ 0/3', {
      fontSize: '12px',
      color: '#ffcc00',
    }).setOrigin(0.5);
    this.infoBestText = this.add.text(0, 5, 'Best: --', {
      fontSize: '11px',
      color: '#aaaaaa',
    }).setOrigin(0.5);
    this.playButton = this.add.text(0, 40, '▶ PLAY', {
      fontSize: '14px',
      color: '#00ffcc',
      backgroundColor: '#2a2a4a',
      padding: { x: 16, y: 6 },
