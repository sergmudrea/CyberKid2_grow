...
// ============================================================================
// ВЫБОР УРОВНЯ – ИСПРАВЛЕННАЯ ЛОГИКА РАЗБЛОКИРОВКИ
// ============================================================================
...
export class LevelSelect extends Scene {
  ...
  init(data: { worldId: string }): void {
    ...
    logger.debug('LevelSelect', 'init', `World: ${this.worldId}`);
  }
  ...
  async create(): Promise<void> {
    ...
    const worldNames: Record<string, string> = {
      meadow: '🌾 MEADOW', ocean: '🌊 OCEAN', clouds: '☁️ CLOUDS',
      fairytale: '🏰 FAIRYTALE', volcano: '🌋 VOLCANO', arcade: '🎮 ARCADE', bonus: '⭐ BONUS',
    };
    const title = this.add.text(width / 2, 50, worldNames[this.worldId] || this.worldId, {
      fontSize: '28px', color: '#00ffcc', fontFamily: 'monospace', stroke: '#0066ff', strokeThickness: 2,
    }).setOrigin(0.5);
    ...
    logger.debug('LevelSelect', 'create', `Loaded ${this.levelIds.length} levels`);

    this.renderLevelGrid();
    this.createInfoPanel();

    const backButton = this.add.text(10, 10, '← BACK', {
      fontSize: '18px', color: '#ffffff', backgroundColor: '#2a2a4a', padding: { x: 16, y: 8 },
    }).setInteractive({ useHandCursor: true });
    backButton.on('pointerdown', () => this.scene.start('WorldMap'));
    backButton.setScrollFactor(0).setDepth(100);

    const worldMapButton = this.add.text(width - 120, 10, '🌍 WORLDS', {
      fontSize: '14px', color: '#ffffff', backgroundColor: '#2a2a4a', padding: { x: 12, y: 6 },
    }).setInteractive({ useHandCursor: true });
    worldMapButton.on('pointerdown', () => this.scene.start('WorldMap'));
    worldMapButton.setScrollFactor(0).setDepth(100);

    gameEvents.on('PROGRESS_UPDATED', () => {
      if (this.isReady && this.cameras && this.cameras.main) {
        this.renderLevelGrid();
        if (this.selectedLevelId) this.updateInfoPanel(this.selectedLevelId);
      }
    });
  }
  ...
  private renderLevelGrid(): void {
    ...
    const cols = 4, rows = 3;
    const startX = 200, startY = 130;
    const spacingX = 90, spacingY = 85;
    ...
    for (let i = 0; i < pageLevels.length; i++) {
      ...
      if (this.worldId === 'arcade') {
        // В Arcade все уровни разблокированы всегда (для тестирования)
        isLocked = false;
      } else {
        if (levelNum === 1) {
          isLocked = false;
      ...
      if (isLocked) {
        ...
        container.on('pointerover', () => { bg.setStrokeStyle(3, 0xffaa00); levelText.setColor('#ffaa00'); });
        container.on('pointerout', () => { bg.setStrokeStyle(2, isCompleted ? 0x44cc44 : 0x00ffcc); levelText.setColor('#ffffff'); });
      }
    ...
    prevBtn.on('pointerdown', () => { if (this.currentPage > 0) { this.currentPage--; this.renderLevelGrid(); } });
    prevBtn.setScrollFactor(0).setDepth(100);
    const nextBtn = this.add.text(this.cameras.main.width - 50, 100, '▶', { fontSize: '24px', color: '#ffffff' }).setInteractive({ useHandCursor: true });
    nextBtn.on('pointerdown', () => { if (this.currentPage + 1 < totalPages) { this.currentPage++; this.renderLevelGrid(); } });
    nextBtn.setScrollFactor(0).setDepth(100);
  }
  ...
  private createInfoPanel(): void {
    ...
    this.infoLevelText = this.add.text(0, -50, 'Level --', { fontSize: '14px', color: '#ffffff' }).setOrigin(0.5);
    ...
    this.playButton.on('pointerdown', () => {
      if (this.selectedLevelId) this.scene.start('GameScene', { levelId: this.selectedLevelId });
    });
